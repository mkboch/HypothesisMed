...
files = []
for p in sorted(Path("results").glob("*medqa_original1000*.jsonl")):
    if "smoke" in p.name:
        continue
    if p.stat().st_size == 0:
        continue
...
rows = [calc(p) for p in files]
df = pd.DataFrame(rows)

if len(df):
    df = df.sort_values(["answer_accuracy", "parse_coverage"], ascending=False)

Path("results/paper_ready_clean").mkdir(parents=True, exist_ok=True)
df.to_csv("results/paper_ready_clean/medqa_original1000_paper_table.csv", index=False)
df.to_json("results/paper_ready_clean/medqa_original1000_paper_table.json", orient="records", indent=2)

print(df.to_string(index=False))
print("\nSaved results/paper_ready_clean/medqa_original1000_paper_table.csv")
