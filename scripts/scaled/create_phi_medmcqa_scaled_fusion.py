...
MODEL = "microsoft_phi_4_mini_instruct"
DATASET_STEM = "medmcqa_main_large"
...
def choose_answer(direct, cot, hyp):
    ...
    # Deterministic fallback order used in the revised paper.
    for name, row in [("direct", direct), ("cot", cot), ("hypothesismed_v3", hyp)]:
        ans = row.get("pred_answer")
        if ans:
...
def main():
    ...
    print()
    print("===== PHI MEDMCQA SCALED FUSION SUMMARY =====")
    print(f"n={n}")
